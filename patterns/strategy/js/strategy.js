...
class InfoContext{
    constructor(strategy, data, element){
        this.setStrategy(strategy)  // unificada la asignacion de estrategia  // informacion a evaluar
        this.data = data
        this.element = element
    }
    setStrategy(strategy){
        this.strategy = strategy  // informacion a evaluar
    }
    show(){
...
class ListStrategy{
    show(data, element){ // metodo creado
        element.innerHTML = data.reduce((ac, e)=>{        
            return ac + `<div>  <h2>${e.name}</h2>  <p>${e.country}<p>  </div>   <hr>`
        }, "")
    }
}
...
class DetailedListStrategy{
    show(data, element){
         element.innerHTML = data.reduce((ac, e)=>{
            return ac + `<div> <h2>${e.name}</h2>   <p>${e.country}<p> <p>${e.info}</p> </div>    <hr>`
        }, "")
    }
}
...
class ListWithImageStrategy{
    show(data, element){
         element.innerHTML = data.reduce((ac, e)=>{
                return ac + `<div> <img width="10%" src="${e.img}"> <h2>${e.name.toUpperCase()}</h2>  <p>${e.country}<p> <p>${e.info}</p>  </div>    <hr>`
        }, "")
    }
}
...
const info = new InfoContext(new ListStrategy(), data, content)  //content es el id en HTML 
